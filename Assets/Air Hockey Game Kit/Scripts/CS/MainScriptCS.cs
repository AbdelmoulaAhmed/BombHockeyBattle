...
public class MainScriptCS : MonoBehaviour {
	...
	public MalletCpuScriptCS p1CpuScript;
	public MalletPlayerScriptCS p1PlayerScript;
	public GameObject player2; // mallet 2
	public MalletCpuScriptCS p2CpuScript;
	public MalletPlayerScriptCS p2PlayerScript;

	public GameObject puck;
	
	public PuckScriptCS puckScript;
	...
	private UiScoreScriptCS uiScoreScript;
	...
	IEnumerator Start ()
	{
		...
		p1CpuScript = player1.GetComponent("MalletCpuScriptCS") as MalletCpuScriptCS;
		p2CpuScript = player2.GetComponent("MalletCpuScriptCS") as MalletCpuScriptCS;
		// Get components "MalletPlayerScript" from each player and assign them to variables "p1PlayerScript" and "p2PlayerScript"
		p1PlayerScript = player1.GetComponent("MalletPlayerScriptCS") as MalletPlayerScriptCS;
		p2PlayerScript = player2.GetComponent("MalletPlayerScriptCS") as MalletPlayerScriptCS;
		
		// Now get the puck's script
		puckScript = puck.GetComponent("PuckScriptCS") as PuckScriptCS;
		...
		uiScoreScript = uiScore.GetComponent("UiScoreScriptCS") as UiScoreScriptCS;
	...
	// Called by UI buttons
	public IEnumerator UiEventOptionButton (int buttonNumber, int optionValue)
	{
	...
	{	
		...
		for(int i=0; i<uiGameOverInfos.Length; i++)	uiGameOverInfos[i].SetActiveRecursively (false);
	...
	IEnumerator SetGameplayPreset ()
	{
		...
		if (gameplayPreset == "arcade")
		{
			...
			puck.rigidbody.mass = 0.01f;
			puck.rigidbody.drag = 0;
			puck.rigidbody.angularDrag = 0.5f;
		...
		if (gameplayPreset == "balanced")
		{
			...
			puckScript.SetMaxVelocity (puckScript.maxVelocity);
			puck.rigidbody.mass = 0.03f;
			puck.rigidbody.drag = 0.1f;
			puck.rigidbody.angularDrag = 0.5f;
		...
		if (gameplayPreset == "simulation")
		{
			...
			puckScript.SetMaxVelocity (puckScript.maxVelocity);
			puck.rigidbody.mass = 0.06f;
			puck.rigidbody.drag = puck.rigidbody.angularDrag = 0.5f;
